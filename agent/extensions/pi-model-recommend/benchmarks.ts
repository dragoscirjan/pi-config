...
export async function syncBenchmarks(): Promise<number> {
    ...
    const refactorUrl = "https://raw.githubusercontent.com/paul-gauthier/aider/main/aider/website/_data/refactor_leaderboard.yml";
    ...
    try {
        ...
        if (res.ok) {
            ...
            for (const block of blocks.slice(1)) {
                const model = block.match(/model:\s*(.*)/)?.[1]?.trim()?.replace(/^"|"$/g, "");
...
export function getAllBenchmarks(): BenchmarkStats[] {
    try {
        return getRouterDb().prepare("SELECT model, pass_rate_edit as editPassRate, pass_rate_refactor as refactorPassRate FROM model_benchmarks").all() as BenchmarkStats[];
    } catch {
        return [];
    }
}
