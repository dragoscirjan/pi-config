...
import type { ModelLike, RecommendConfig, Intent, RecommendOptions, TaxonomyState, ScoredModel, CapabilityConstraints } from "./types";
...
function parseRecommendArgs(raw: string): RecommendOptions {
	...
	const pushProviders = (value: string | undefined) => {
		if (!value) return;
		for (const part of value.split(",").map((p) => p.trim().toLowerCase()).filter(Boolean)) opts.providers.push(part);
	};
...
function findModelFromInput(input: string, registryModels: ModelLike[]): ModelLike | undefined {
	...
	return registryModels.find((m) => m.id.toLowerCase() === raw) ?? registryModels.find((m) => canonicalFamily(m.id) === canonicalFamily(raw)) ?? registryModels.find((m) => `${m.provider}/${m.id}`.toLowerCase().includes(raw));
}
...
export default function modelRecommendExtension(pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		if (!(ctx as any).hasUI) {
			const lines = top.slice(0, 5).map((m, i) => `  ${i + 1}. ${m.provider}/${m.model}  score=${m.score.toFixed(1)} reason=${m.reasoning} intel=${m.intelligence}`);
		...
		listItems.push({ value: "keep", label: `⟳  Keep current  (${currentModel})`, description: "Continue with currently active model" });
		listItems.push({ value: "custom", label: "✎  Enter custom model...", description: "Type provider/model-id manually" });
		...
		const choice = await (ctx as any).ui.custom<string | null>((tui: any, theme: any, _kb: any, done: (v: string | null) => void) => {
			...
			container.addChild(new Text(theme.fg("dim", ` Task intent: ${Array.from(intent.domains).join("/") || "general"} · complexity ${intent.complexity}/100`), 1, 0));
