...
export async function enrichTaxonomyWithLiveSignals(taxonomy: Taxonomy, config: RecommendConfig, sourceOverride?: string): Promise<{ taxonomy: Taxonomy; liveSources: string[]; enriched: boolean }> {
...
export async function ensureTaxonomy(rebuild: boolean, liveTaxonomy: boolean, config: RecommendConfig, liveSourceOverride?: string): Promise<TaxonomyState> {
	let rebuilt = false;
	if (rebuild) {
...
export function mergeTaxonomyFromPath(path: string, policy: "append" | "replace" | "keep"): { categories: number; concepts: number; terms: number } {
