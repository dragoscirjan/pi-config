...
export function deriveConstraints(intent: Intent, config: RecommendConfig): CapabilityConstraints {
  ...
  const minIntel = clamp(
    Math.round(36 + n.reasoningDepth * 36 + n.correctnessRisk * 22 + n.safetyCriticality * 14 + n.bestQualityBias * 10),
    30,
    99,
  );
  const minReasoning = clamp(Math.round(18 + n.reasoningDepth * 56 + n.safetyCriticality * 12), 10, 98);
  const minToolReliability = clamp(
    Math.round(28 + n.correctnessRisk * 36 + n.systemBreadth * 24 + n.safetyCriticality * 10),
    20,
    99,
  );
  const minContext =
    n.contextVolume >= 0.9
      ? 512_000
      : n.contextVolume >= 0.75
        ? 256_000
        : n.contextVolume >= 0.55
          ? 128_000
          : n.contextVolume >= 0.35
            ? 64_000
            : 16_000;
  ...
  if (n.costSensitivity >= 0.9 && intent.complexity >= 60) {
    return {
      minIntel: Math.max(minIntel, config.defaults.minIntelForComplexCheap),
      minReasoning,
      minToolReliability,
      minContext,
      requireReasoning,
...
export function scoreModel(
  ...
  const ctxFit = clamp(
    (Math.log2(Math.max(8_000, model.contextWindow)) - Math.log2(constraints.minContext)) / 3 + 0.55,
    0,
    1,
  );
  ...
  const priceWeight = clamp(
    0.33 + n.costSensitivity * 0.42 - n.bestQualityBias * 0.18 - n.safetyCriticality * 0.12,
    0.12,
    0.82,
  );
  ...
  model.breakdown = {
    ...
    weights: {
      intel: constraints.minIntel,
      speed: n.latencySensitivity,
      price: priceWeight,
      context: constraints.minContext,
    },
    reasons: [
...
export function applyCapabilityDeltaGuard(
  models: ScoredModel[],
  intent: Intent,
  config: RecommendConfig,
): ScoredModel[] {
