...
function getAuthenticatedProvidersFromAuthJson(): Set<string> {
	...
	try {
		...
		for (const [provider, cfg] of Object.entries(json)) {
			...
			if (p === "github-copilot" || Boolean(cfg.key ?? cfg.apiKey) || Boolean(cfg.access ?? cfg.refresh ?? cfg.token)) providers.add(provider);
		}
		return providers;
		...
		handler: async (args, ctx) => {
			...
			for (let i = 0; i < tokens.length; i++) {
				...
				if (t === "--grep" || t === "-g") parsed.grep = tokens[++i]?.toLowerCase() ?? "";
					...
					if (maybeDir === "asc" || maybeDir === "desc") { parsed.desc = maybeDir === "desc"; i++; }
				}
			...
			if (parsed.providers.length > 0) rows = rows.filter((r) => parsed.providers.some((p) => r.provider.toLowerCase().includes(p.toLowerCase())));
			...
			rows.sort((a, b) => {
				...
				switch (parsed.sortBy) {
					...
					case "intel": cmp = a.intel - b.intel; break;
					case "price": cmp = a.costOut - b.costOut; break;
			...
			const priceStr = (r: typeof rows[0]) => `${formatMoney(r.costIn)} / ${formatMoney(r.costOut)}${r.priceEstimated ? " ~" : ""}`;
