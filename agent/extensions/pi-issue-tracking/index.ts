...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params: any, signal, onUpdate, ctx) {
      const issuesDir = path.join(ctx.cwd, ".issues");
      if (!fs.existsSync(issuesDir)) return { content: [{ type: "text", text: "No issues." }], details: { issues: [] } as any };
      const issues: string[] = [];
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params: any, signal, onUpdate, ctx) {
      ...
      return { content: [{ type: "text", text: fs.readFileSync(path.join(issuesDir, fn), "utf-8") }], details: { file: fn } as any };
    },
  });
